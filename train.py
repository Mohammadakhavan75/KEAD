...
def train(loader, net, train_global_iter, criterion, optimizer, device, writer):
    ...
    for inputs, targets in tqdm(loader):
        ...
        optimizer.zero_grad()
        preds, features_list = net(inputs, True)
        probs = torch.softmax(preds, dim=1)

        # Calculate loss contrastive for layer
        loss_low_level = torch.zeros(features_list[-1].shape[1])
        for i in range(features_list[-1].shape[1]):
            single_filter = features_list[-1][:,i,:,:]
        loss_contrast = contrastive()
