...
for i, data in enumerate(tqdm(loader)):
    ...
    if len(imgs_n) != len(imgs_aug):
        ...
        diffs.extend(torch.sum(torch.pow((text_n_features - text_aug_features), 2), dim=1).detach().cpu().numpy())
...
with open(f'./tensors3/diffs_{args.aug}.pkl', 'wb') as f:
    pickle.dump(diffs, f)
with open(f'./tensors3/diffs_target_{args.aug}.pkl', 'wb') as f:
